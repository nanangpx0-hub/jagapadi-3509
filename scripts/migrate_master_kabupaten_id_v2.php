<?php
/**
 * Migration script v2 to modify master_kabupaten.id column
 * - Change id to VARCHAR(2)
 * - Add UNIQUE constraint
 * - Update id values with last 2 digits of kode_kabupaten
 * - Handle duplicates properly by using full kabupaten codes
 * 
 * Usage:
 * php migrate_master_kabupaten_id_v2.php [--force|--yes]
...
echo "=== Migration v2: master_kabupaten.id column modification ===\n\n";
...
try {
    ...
    try {
        ...
        // Step 9: Create mapping table for new IDs
        echo "\nStep 9: Creating unique ID mapping...\n";
        
        // Get all records and create unique IDs
        ...
        foreach ($all_records as $record) {
            $base_id = substr($record['kode_kabupaten'], -2);
            $new_id = $base_id;
            $counter = 0;
            
            // Ensure ID is exactly 2 characters
            if (strlen($new_id) < 2) {
                $new_id = str_pad($new_id, 2, '0', STR_PAD_LEFT);
            }
            
            // If this ID is already used, try alternatives
            while (isset($used_ids[$new_id])) {
                $counter++;
                // Generate alternatives that are exactly 2 characters
                // Use letters if we run out of numbers
                if ($counter <= 99) {
                    $new_id = str_pad((string)$counter, 2, '0', STR_PAD_LEFT);
                } else {
                    // Use letters A-Z for numbers 100+
                    $letter_index = ($counter - 100) % 26;
                    $letter = chr(65 + $letter_index); // A-Z
                    $number_part = floor(($counter - 100) / 26);
                    $new_id = $letter . str_pad((string)$number_part, 1, '0', STR_PAD_LEFT);
                }
            }
            ...
            echo "  Mapping: {$record['kode_kabupaten']} (old ID: {$record['id']}) -> new ID: $new_id\n";
        }
    ...
    $migration_record = [
        'migration' => 'migrate_master_kabupaten_id_v2',
