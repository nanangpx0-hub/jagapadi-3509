    ...
    <div class="col-md-10 offset-md-1">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="fas fa-plus"></i> Buat Laporan Lainnya Baru
            ...
            <form action="<?= BASE_URL ?>laporan-lainnya/store" method="POST" enctype="multipart/form-data" id="formCreateLaporan">
                ...
                <div class="card-body">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="jenis_id" id="jenisSelect" class="form-control" required autocomplete="off">
                                    ...
                                    <option value="<?= $jl['id'] ?>"
                                        data-fields='<?= htmlspecialchars($jl['fields_json'] ?? '[]') ?>'>
                    ...
                    <div id="dynamicFieldsContainer" class="mb-3">
                        ...
                        <p class="text-muted">Pilih jenis laporan terlebih dahulu untuk menampilkan field yang relevan.</p>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="deskripsi" id="deskripsiTextarea" class="form-control" rows="3" placeholder="Deskripsi kondisi, gejala, atau informasi tambahan"></textarea>
                    </div>
                ...
                <div class="card-footer">
                    ...
                    <a href="<?= BASE_URL ?>laporan-lainnya" class="btn btn-secondary" id="btnCancel">
...
// Dynamic fields renderer (berdasarkan fields_json pada option jenis laporan)
function renderDynamicFields() {
    const jenisSelect = document.getElementById('jenisSelect');
    const dynamicContainer = document.getElementById('dynamicFieldsContainer');

    if (!jenisSelect || !dynamicContainer) return;

    const selectedOption = jenisSelect.options[jenisSelect.selectedIndex];
    const fieldsJson = selectedOption ? selectedOption.getAttribute('data-fields') : null;

    if (!fieldsJson) {
        dynamicContainer.innerHTML = '<h5><i class="fas fa-list"></i> Data Laporan</h5><p class="text-muted">Pilih jenis laporan terlebih dahulu untuk menampilkan field yang relevan.</p>';
        return;
    }
    ...
    if (!Array.isArray(fields) || !fields.length) {
        dynamicContainer.innerHTML = '<h5><i class="fas fa-list"></i> Data Laporan</h5><p class="text-muted">Tidak ada field tambahan untuk jenis ini.</p>';
        return;
    }

    let html = '<h5><i class="fas fa-list"></i> Data Laporan</h5><div class="row">';
    ...
    fields.forEach(function(field) {
        ...
        html += '<label>' + label + ' ' + isRequired + '</label>';

        if (field.type === 'number') {
            html += '<input type="number" name="' + name + '" class="form-control" ' + requiredAttr + ' step="any" min="0" placeholder="Masukkan ' + label + '">';
        } else if (field.type === 'date') {
            html += '<input type="date" name="' + name + '" class="form-control" ' + requiredAttr + '>';
        } else {
            html += '<input type="text" name="' + name + '" class="form-control" ' + requiredAttr + ' placeholder="Masukkan ' + label + '">';
        }
...
document.querySelector('form').addEventListener('submit', function(e) {
    ...
    const requiredFields = [
        { name: 'jenis_id', label: 'Jenis Laporan' },
        { name: 'tanggal_kejadian', label: 'Tanggal Kejadian' },
    ...
    // Validate dynamic fields (required fields rendered from jenis laporan)
    const dynamicContainer = document.getElementById('dynamicFieldsContainer');
    if (dynamicContainer) {
        const dynamicRequired = dynamicContainer.querySelectorAll('input[required]');
        for (let input of dynamicRequired) {
