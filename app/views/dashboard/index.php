    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-map-marked-alt text-success"></i> Peta Sebaran Hama</h3>
                <div class="card-tools">
                    <a href="<?= BASE_URL ?>dashboard/map" class="btn btn-sm btn-success">
                        <i class="fas fa-expand"></i> Peta Lengkap
                    </a>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ---- Map Section ----
    const mapHome = L.map('dashboardMapHome', {
    ...
    const hamaClusterHome = L.markerClusterGroup({
        chunkedLoading: true,
        spiderfyOnMaxZoom: true,
        showCoverageOnHover: false,
        maxClusterRadius: 50
    });
    mapHome.addLayer(hamaClusterHome);
    fetch('<?= BASE_URL ?>api/dashboard/map/hama?year=' + new Date().getFullYear() + '&status=')
        .then(r => r.json())
        .then(data => {
            if (!data.success) return;
            document.getElementById('statHamaHome').textContent = data.count;
            (data.data.features || []).forEach(f => {
                const c = f.geometry.coordinates;
                const p = f.properties;
                const color = p.tingkat_keparahan === 'Berat' ? '#dc3545' : p.tingkat_keparahan === 'Sedang' ? '#ffc107' : '#198754';
    ...
    fetch('<?= BASE_URL ?>api/dashboard/map/irigasi')
        .then(r => r.json())
        .then(d => {
            if (d.success) document.getElementById('statIrigasiHome').textContent = d.count;
        })
        .catch(() => {});
    fetch('<?= BASE_URL ?>api/dashboard/map/weather')
        .then(r => r.json())
        .then(d => {
            if (d.success && d.data.rainfall) document.getElementById('statRainfallHome').textContent = d.data.rainfall.length;
        })
        .catch(() => {});
    fetch('<?= BASE_URL ?>api/dashboard/map/hamaSummary?year=' + new Date().getFullYear())
        .then(r => r.json())
        .then(d => {
            if (d.success) document.getElementById('statKecamatanHome').textContent = d.count;
        })
        .catch(() => {});
});
</script>
