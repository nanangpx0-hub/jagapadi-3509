...
import '../providers/laporan_alat_sarana_provider.dart';

class AlatSaranaFormScreen extends StatefulWidget {
  final int? id;
  const AlatSaranaFormScreen({super.key, this.id});

  @override
  State<AlatSaranaFormScreen> createState() => _AlatSaranaFormScreenState();
}

class _AlatSaranaFormScreenState extends State<AlatSaranaFormScreen> {
  final _formKey = GlobalKey<FormState>();
  final _tanggalCtrl = TextEditingController();
  final _namaAlatCtrl = TextEditingController();
  final _kapasitasCtrl = TextEditingController();
  final _tahunPengadaanCtrl = TextEditingController();
  final _catatanCtrl = TextEditingController();
  final _latCtrl = TextEditingController();
  final _lngCtrl = TextEditingController();

  int? _kabId, _kecId, _desaId;
  String? _jenisSarana, _kondisi;
  File? _foto;
  ...
  void initState() {
    ...
    if (widget.id != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        final p = context.read<LaporanAlatSaranaProvider>();
        ...
        if (d != null) {
          setState(() {
            ...
            _desaId = d.desaId;
            _namaAlatCtrl.text = d.namaAlat ?? '';
            _jenisSarana = d.jenisSarana;
            _kondisi = d.kondisi;
            _kapasitasCtrl.text = d.kapasitas ?? '';
            _tahunPengadaanCtrl.text = d.tahunPengadaan?.toString() ?? '';
  ...
  Future<bool> _saveDraft() async {
    ...
    final data = <String, dynamic>{
      ...
      'jenis_sarana': _jenisSarana,
      'kondisi': _kondisi,
      'kapasitas': _kapasitasCtrl.text,
      if (_tahunPengadaanCtrl.text.isNotEmpty)
        'tahun_pengadaan': int.tryParse(_tahunPengadaanCtrl.text),
          ...
          if (_foto != null) {
            final upload = await showUploadProgress(
              ...
              task: (onProgress) => p.api.uploadFoto(
                '/laporan-alat-sarana/$newId/foto',
                _foto!.path,
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
                'Apakah Anda yakin ingin mengirim laporan alat & sarana ini?'),
            const SizedBox(height: 12),
            Text('â€¢ Tanggal: ${_tanggalCtrl.text}'),
            Text('â€¢ Nama Alat: ${_namaAlatCtrl.text}'),
            Text('â€¢ Jenis: ${_jenisSarana ?? "-"}'),
          ],
        ),
        actions: [
          TextButton(
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.id != null
            ? 'Edit Laporan Alat & Sarana'
            : 'Laporan Alat & Sarana Baru'),
      ),
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: 'Nama Alat / Sarana',
                  errorText: _fe('nama_alat'),
                ),
              ...
              DropdownButtonFormField<String>(
                decoration: InputDecoration(
                  labelText: 'Jenis Sarana',
                  errorText: _fe('jenis_sarana'),
                ),
                initialValue: _jenisSarana,
                items: [
                  'Traktor',
                  'Pompa Air',
                  'Gudang',
                  ...
                  'Lainnya'
                    ...
                    .toList(),
                onChanged: (v) => setState(() => _jenisSarana = v),
              ),
              const SizedBox(height: 16),
              DropdownButtonFormField<String>(
                decoration: InputDecoration(
                  labelText: 'Kondisi',
                  errorText: _fe('kondisi'),
                ),
                initialValue: _kondisi,
                items: ['Baik', 'Rusak Ringan', 'Rusak Berat', 'Tidak Layak']
                    .map((k) => DropdownMenuItem(value: k, child: Text(k)))
                    .toList(),
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: TextFormField(
                      controller: _tahunPengadaanCtrl,
                      decoration: InputDecoration(
                        labelText: 'Tahun Pengadaan',
                        errorText: _fe('tahun_pengadaan'),
                      ),
