...
class CuacaListScreen extends StatefulWidget {
  final String? initialStatus;
  const CuacaListScreen({super.key, this.initialStatus});

  @override
  State<CuacaListScreen> createState() => _CuacaListScreenState();
}

class _CuacaListScreenState extends State<CuacaListScreen> {
  ...
  Future<void> _refresh() async {
    await context.read<LaporanCuacaProvider>().loadList(
          refresh: true,
          status: _status,
  ...
  String _emptyStateMessage() {
    ...
    switch (_status) {
      case 'Draf':
        return 'Belum ada draf laporan cuaca. Buat laporan baru dengan tombol +';
      case 'Submitted':
        return 'Tidak ada laporan cuaca yang sedang menunggu verifikasi';
      case 'Diverifikasi':
        return 'Belum ada laporan cuaca yang diverifikasi';
      case 'Ditolak':
        return 'Tidak ada laporan cuaca yang ditolak';
      default:
        return 'Belum ada data laporan cuaca';
    }
  }

  @override
  Widget build(BuildContext context) {
    final p = context.watch<LaporanCuacaProvider>();
    return Scaffold(
      appBar: AppBar(
        title: _isSearching
            ? TextField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Cari laporan cuaca...',
                  hintStyle: TextStyle(color: Colors.white70),
                  border: InputBorder.none,
                ),
                onChanged: _onSearchChanged,
              )
            : const Text('Laporan Cuaca'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            onPressed: () => context.push('/cuaca/create').then((_) => _refresh()),
          ),
        ],
      ),
      body: Column(
        children: [
          _StatusFilter(
            ...
            onChange: (s) {
              setState(() => _status = s);
              context.read<LaporanCuacaProvider>().loadList(
                    refresh: true,
                    status: s,
                    search: _searchCtrl.text,
                  );
          ...
          LocalDraftsBanner(
            type: 'cuaca',
            api: context.read<LaporanCuacaProvider>().api,
                        ...
                        : RefreshIndicator(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (_, i) {
                                ...
                                return _LaporanCard(
                                  item: item,
                                  onTap: () => context.push('/cuaca/${item.id}').then((_) => _refresh()),
                                );
                              },
                            ),
                          ),
