...
class CuacaDetailScreen extends StatefulWidget {
  final int id;
  const CuacaDetailScreen({super.key, required this.id});

  @override
  State<CuacaDetailScreen> createState() => _CuacaDetailScreenState();
}

class _CuacaDetailScreenState extends State<CuacaDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
        (_) => context.read<LaporanCuacaProvider>().loadDetail(widget.id));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(l?.nomorLaporan ?? 'Detail Laporan Cuaca'),
        actions: [
          if (l != null &&
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (l.tanggal != null)
                                _InfoRow('Tanggal', l.tanggal!),
                              if (l.suhuMin != null)
                                _InfoRow('Suhu Min', '${l.suhuMin} °C'),
                              if (l.suhuMax != null)
                                _InfoRow('Suhu Max', '${l.suhuMax} °C'),
                              if (l.curahHujan != null)
                                _InfoRow('Curah Hujan', '${l.curahHujan} mm'),
                              if (l.kelembaban != null)
                                _InfoRow('Kelembaban', '${l.kelembaban} %'),
                              if (l.kecepatanAngin != null)
                                _InfoRow('Kecepatan Angin',
                                    '${l.kecepatanAngin} km/j'),
                              if (l.kondisiCuaca != null)
                                _InfoRow('Kondisi Cuaca', l.kondisiCuaca!),
