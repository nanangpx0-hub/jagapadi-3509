...
class PupukListScreen extends StatefulWidget {
  final String? initialStatus;
  const PupukListScreen({super.key, this.initialStatus});

  @override
  State<PupukListScreen> createState() => _PupukListScreenState();
}

class _PupukListScreenState extends State<PupukListScreen> {
  ...
  Future<void> _refresh() async {
    await context.read<LaporanPupukProvider>().loadList(
          refresh: true,
          status: _status,
  ...
  String _emptyStateMessage() {
    ...
    switch (_status) {
      case 'Draf':
        return 'Belum ada draf laporan pupuk. Buat laporan baru dengan tombol +';
      case 'Submitted':
        return 'Tidak ada laporan pupuk yang sedang menunggu verifikasi';
      case 'Diverifikasi':
        return 'Belum ada laporan pupuk yang diverifikasi';
      case 'Ditolak':
        return 'Tidak ada laporan pupuk yang ditolak';
      default:
        return 'Belum ada data laporan pupuk';
    }
  }
        ...
        title: _isSearching
            ? TextField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Cari laporan pupuk...',
                  hintStyle: TextStyle(color: Colors.white70),
                  border: InputBorder.none,
                ),
                onChanged: _onSearchChanged,
              )
            : const Text('Laporan Pupuk'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            onPressed: () => context.push('/pupuk/create').then((_) => _refresh()),
          ),
        ],
      ),
      body: Column(
        children: [
          _StatusFilter(
            ...
            onChange: (s) {
              setState(() => _status = s);
              context.read<LaporanPupukProvider>().loadList(
                    refresh: true,
                    status: s,
                    search: _searchCtrl.text,
                  );
          ...
          LocalDraftsBanner(
            type: 'pupuk',
            api: context.read<LaporanPupukProvider>().api,
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return _LaporanCard(
                              item: item,
                              onTap: () => context.push('/pupuk/${item.id}').then((_) => _refresh()),
                            );
                          },
                        ),
                      ),
