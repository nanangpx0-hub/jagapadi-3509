...
class PupukFormScreen extends StatefulWidget {
  final int? id;
  const PupukFormScreen({super.key, this.id});

  @override
  State<PupukFormScreen> createState() => _PupukFormScreenState();
}

class _PupukFormScreenState extends State<PupukFormScreen> {
  ...
  String? _jenisPupuk, _metodeAplikasi;
  File? _foto;
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        if (d != null) {
          setState(() {
            ...
            _jenisPupuk = d.jenisPupuk;
            _dosisCtrl.text = d.dosisPerHa?.toString() ?? '';
            _luasCtrl.text = d.luasPemupukan?.toString() ?? '';
            _metodeAplikasi = d.metodeAplikasi;
  ...
  Future<bool> _saveDraft() async {
    ...
    final data = <String, dynamic>{
      ...
      'jenis_pupuk': _jenisPupuk,
      if (_dosisCtrl.text.isNotEmpty)
        'dosis_per_ha': double.tryParse(_dosisCtrl.text),
      if (_luasCtrl.text.isNotEmpty)
        'luas_pemupukan': double.tryParse(_luasCtrl.text),
      'metode_aplikasi': _metodeAplikasi,
      'kabupaten_id': _kabId,
      'kecamatan_id': _kecId,
      'desa_id': _desaId,
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text('Apakah Anda yakin ingin mengirim laporan pupuk ini?'),
            const SizedBox(height: 12),
            Text('â€¢ Tanggal: ${_tanggalCtrl.text}'),
            Text('â€¢ Jenis Pupuk: ${_jenisPupuk ?? "-"}'),
            Text(
                'â€¢ Dosis: ${_dosisCtrl.text.isNotEmpty ? "${_dosisCtrl.text} kg/ha" : "-"}'),
            Text(
                'â€¢ Luas Pemupukan: ${_luasCtrl.text.isNotEmpty ? "${_luasCtrl.text} ha" : "-"}'),
          ],
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
            widget.id != null ? 'Edit Laporan Pupuk' : 'Buat Laporan Pupuk'),
      ),
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: ['Urea', 'NPK', 'Organik', 'Kompos', 'Lainnya']
                    ...
                    .toList(),
                onChanged: (v) => setState(() => _jenisPupuk = v),
                validator: (_) =>
                    _jenisPupuk == null ? 'Jenis pupuk wajib dipilih' : null,
              ),
              const SizedBox(height: 16),
              Row(
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Dosis (kg/ha)',
                        errorText: _fe('dosis_per_ha'),
                      ),
                      keyboardType:
                          const TextInputType.numberWithOptions(decimal: true),
                      validator: (v) => LaporanValidators.angka(v,
                          nonNegative: true, label: 'Dosis'),
                    ),
                  ),
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Luas (ha)',
                        errorText: _fe('luas_pemupukan'),
                      ),
                      keyboardType:
                          const TextInputType.numberWithOptions(decimal: true),
                      validator: (v) => LaporanValidators.angka(v,
                          nonNegative: true, label: 'Luas'),
                    ),
                  ),
                ],
              ),
              ...
              DropdownButtonFormField<String>(
                ...
                items: ['Tabur', 'Kocor', 'Semprot', 'Injeksi']
                    .map((k) => DropdownMenuItem(value: k, child: Text(k)))
                    .toList(),
                onChanged: (v) => setState(() => _metodeAplikasi = v),
              ),
              const SizedBox(height: 16),
              WilayahPicker(
                kabupatenId: _kabId,
