...
class PanenListScreen extends StatefulWidget {
  final String? initialStatus;
  const PanenListScreen({super.key, this.initialStatus});

  @override
  State<PanenListScreen> createState() => _PanenListScreenState();
}

class _PanenListScreenState extends State<PanenListScreen> {
  ...
  Future<void> _refresh() async {
    await context.read<LaporanPanenProvider>().loadList(
          refresh: true,
          status: _status,
  ...
  String _emptyStateMessage() {
    ...
    switch (_status) {
      case 'Draf':
        return 'Belum ada draf laporan panen. Buat laporan baru dengan tombol +';
      case 'Submitted':
        return 'Tidak ada laporan panen yang sedang menunggu verifikasi';
      case 'Diverifikasi':
        return 'Belum ada laporan panen yang diverifikasi';
      case 'Ditolak':
        return 'Tidak ada laporan panen yang ditolak';
      default:
        return 'Belum ada data laporan panen';
    }
  }
        ...
        title: _isSearching
            ? TextField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Cari laporan panen...',
                  hintStyle: TextStyle(color: Colors.white70),
                  border: InputBorder.none,
                ),
                onChanged: _onSearchChanged,
              )
            : const Text('Laporan Panen'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            onPressed: () => context.push('/panen/create').then((_) => _refresh()),
          ),
        ],
      ),
      body: Column(
        children: [
          _StatusFilter(
            ...
            onChange: (s) {
              setState(() => _status = s);
              context.read<LaporanPanenProvider>().loadList(
                    refresh: true,
                    status: s,
                    search: _searchCtrl.text,
                  );
          ...
          LocalDraftsBanner(
            type: 'panen',
            api: context.read<LaporanPanenProvider>().api,
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return _LaporanCard(
                              item: item,
                              onTap: () => context.push('/panen/${item.id}').then((_) => _refresh()),
                            );
                          },
                        ),
                      ),
