...
class PanenDetailScreen extends StatefulWidget {
  final int id;
  const PanenDetailScreen({super.key, required this.id});

  @override
  State<PanenDetailScreen> createState() => _PanenDetailScreenState();
}

class _PanenDetailScreenState extends State<PanenDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
        (_) => context.read<LaporanPanenProvider>().loadDetail(widget.id));
  }
  ...
  Widget build(BuildContext context) {
    final p = context.watch<LaporanPanenProvider>();
    final auth = context.watch<AuthProvider>();
    final l = p.detail;

    return Scaffold(
      appBar: AppBar(
        title: Text(l?.nomorLaporan ?? 'Detail Laporan Panen'),
        actions: [
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (l.tanggal != null) _InfoRow('Tanggal Panen', l.tanggal!),
                              if (l.komoditas != null) _InfoRow('Komoditas', l.komoditas!),
                              if (l.varietas != null) _InfoRow('Varietas', l.varietas!),
                              if (l.luasPanen != null) _InfoRow('Luas Panen', '${l.luasPanen} ha'),
                              if (l.hasilPanen != null) _InfoRow('Hasil Panen', '${l.hasilPanen} ton'),
                              if (l.produktivitas != null) _InfoRow('Produktivitas', '${l.produktivitas} ton/ha'),
                              if (l.musimTanam != null) _InfoRow('Musim Tanam', l.musimTanam!),
                            ...
                            children: [
                              Expanded(
                                child: OutlinedButton.icon(
                                  onPressed: () => context.push('/panen/${widget.id}/edit').then((_) => p.loadDetail(widget.id)),
