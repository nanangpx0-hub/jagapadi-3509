...
{
    private const ALLOWED_FILTERS = ['status', 'tanggal_from', 'tanggal_to', 'kabupaten_id', 'kecamatan_id', 'desa_id', 'komoditas', 'q'];

    protected static function table(): string
    {
        return 'laporan_panen';
    }
    ...
    public static function findAccessibleById(int $id, array $currentUser): ?array
    {
        ...
        [$clause, $scopeParams] = ReportAuthorizationPolicy::accessibleCondition('lp', $currentUser);
        if ($clause !== '') {
            $sql .= ' AND ' . $clause;
        }
    ...
    {
        $pdo = self::db();
        $stmt = $pdo->prepare("DELETE FROM `laporan_panen` WHERE `id` = ? AND `user_id` = ? AND `status` = 'Draf'");
