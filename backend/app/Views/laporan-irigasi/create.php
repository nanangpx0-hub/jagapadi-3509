...
<div class="form-card">
    <h2 style="margin-bottom:20px;">Buat Laporan Irigasi</h2>

    <form method="POST" action="/laporan-irigasi">
        <?= \App\Core\Security::csrfField() ?>

        <div class="inline-group">
            <div class="form-group">
                <label for="tanggal">Tanggal</label>
                <input type="date" id="tanggal" name="tanggal" value="<?= \App\Core\Security::e($oldInput['tanggal'] ?? '') ?>">
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama_saluran" name="nama_saluran" maxlength="200" value="<?= \App\Core\Security::e($oldInput['nama_saluran'] ?? '') ?>">
            ...
            <div class="form-group">
                ...
                <select id="kabupaten_id" name="kabupaten_id" onchange="loadKecamatan(this.value)">
                    ...
                    <?php foreach ($kabupaten as $k): ?>
                        <option value="<?= (int) $k['id'] ?>" <?= ($oldInput['kabupaten_id'] ?? '') == $k['id'] ? 'selected' : '' ?>><?= \App\Core\Security::e($k['nama_kabupaten']) ?></option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="inline-group">
            <div class="form-group">
                ...
                <select id="kondisi_fisik" name="kondisi_fisik">
                    ...
                    <option value="Bagus" <?= ($oldInput['kondisi_fisik'] ?? '') === 'Bagus' ? 'selected' : '' ?>>Bagus</option>
                    <option value="Sedang" <?= ($oldInput['kondisi_fisik'] ?? '') === 'Sedang' ? 'selected' : '' ?>>Sedang</option>
                    <option value="Tidak Bagus" <?= ($oldInput['kondisi_fisik'] ?? '') === 'Tidak Bagus' ? 'selected' : '' ?>>Tidak Bagus</option>
                    <option value="Rusak" <?= ($oldInput['kondisi_fisik'] ?? '') === 'Rusak' ? 'selected' : '' ?>>Rusak</option>
                </select>
            ...
            <div class="form-group">
                ...
                <select id="debit_air" name="debit_air">
                    <option value="">-- Pilih --</option>
                    <option value="Cukup" <?= ($oldInput['debit_air'] ?? '') === 'Cukup' ? 'selected' : '' ?>>Cukup</option>
                    <option value="Kurang" <?= ($oldInput['debit_air'] ?? '') === 'Kurang' ? 'selected' : '' ?>>Kurang</option>
                    <option value="Kering" <?= ($oldInput['debit_air'] ?? '') === 'Kering' ? 'selected' : '' ?>>Kering</option>
                </select>
        ...
        <div class="form-group">
            ...
            <input type="text" id="daerah_irigasi" name="daerah_irigasi" maxlength="200" value="<?= \App\Core\Security::e($oldInput['daerah_irigasi'] ?? '') ?>">
        ...
        <div class="inline-group">
            <div class="form-group">
                ...
                <input type="text" id="latitude" name="latitude" placeholder="-8.2011" value="<?= \App\Core\Security::e($oldInput['latitude'] ?? '') ?>">
                <?php if (!empty($errors['latitude'])): ?><div class="error"><?= \App\Core\Security::e($errors['latitude']) ?></div><?php endif; ?>
            </div>
            <div class="form-group">
                <label for="longitude">Longitude</label>
                <input type="text" id="longitude" name="longitude" placeholder="113.6890" value="<?= \App\Core\Security::e($oldInput['longitude'] ?? '') ?>">
        ...
        <div class="form-group">
            ...
            <textarea id="catatan" name="catatan" rows="3"><?= \App\Core\Security::e($oldInput['catatan'] ?? '') ?></textarea>
        ...
        <div class="action-group">
            ...
            <a href="/laporan-irigasi" class="btn-secondary">Batal</a>
        </div>
    </form>
</div>
...
(function() {
    const kabId = '<?= \App\Core\Security::e($oldInput["kabupaten_id"] ?? "") ?>';
    if (kabId) {
        loadKecamatan(kabId);
    }
})();
