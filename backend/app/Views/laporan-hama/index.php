<style>
    ...
    .btn-light { display:inline-block; padding:8px 16px; background:#2e7d32; color:#fff; border-radius:6px; text-decoration:none; font-size:14px; font-weight:500; }
...
<div class="header-bar">
    ...
    <div style="display:flex;gap:8px;">
        <?php if ($currentUser['role'] === 'petugas'): ?>
            <a href="/laporan-hama/create-light" class="btn-light">+ Cepat</a>
            <a href="/laporan-hama/create" class="btn-add">+ Lengkap</a>
        <?php endif; ?>
    </div>
</div>
...
<table>
    ...
    <?php foreach ($data as $lh): ?>
        <tr>
            <td><span class="badge badge-light">#<?= (int) $lh['id'] ?></span></td>
            <td>
                <?php if (!empty($lh['foto_url'])): ?>
                    <img src="<?= \App\Core\Security::e($lh['foto_url']) ?>" alt="Foto" style="width:45px;height:45px;object-fit:cover;border-radius:4px;">
                <?php else: ?>
            ...
            <td><?= \App\Core\Security::e((string)($lh['populasi'] ?? '0')) ?></td>
            <td><span class="badge badge-<?= strtolower($lh['status']) ?>"><?= \App\Core\Security::e($lh['status']) ?></span></td>
            <td><?= \App\Core\Security::e($lh['pelapor_nama'] ?? $lh['pelapor_username'] ?? '-') ?></td>
    ...
    <?php endforeach; ?>
    <?php if (count($data) === 0): ?>
        <tr><td colspan="13" style="text-align:center;color:#999;padding:24px;">Belum ada laporan hama.</td></tr>
    <?php endif; ?>
    </tbody>
</table>
