...
<div class="form-card">
    ...
    <h3 style="margin-bottom:12px;">Foto OPT</h3>

    <?php if (!empty($data['foto_url'])): ?>
        <div style="margin-bottom:12px;">
            <img src="/<?= \App\Core\Security::e($data['foto_url']) ?>" alt="Foto OPT" style="max-width:300px;max-height:200px;border-radius:6px;border:1px solid #e0e0e0;">
            <form method="POST" action="/opt/<?= (int) $data['id'] ?>/foto/delete" style="margin-top:8px;">
                <?= \App\Core\Security::csrfField() ?>
    ...
    <form method="POST" action="/opt/<?= (int) $data['id'] ?>/foto" enctype="multipart/form-data">
        <?= \App\Core\Security::csrfField() ?>
        <div class="form-group">
            <label for="foto">Upload Foto Baru (maks. 5 MB, JPEG/PNG/WebP)</label>
